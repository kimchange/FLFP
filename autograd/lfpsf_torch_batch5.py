...
class PsfGenerator5D:
    ...
    def coherent_psf_full(self, u, v, d, phi, normalized=False, piston_tip_tilt=False):
        Nnum = self.Nnum
    ...
    def calcRMS(self, phi, normalized=True, piston_tip_tilt=True):
    ...
    def incoherent_psf(self, a, d, phi, normalized=False,
                       piston_tip_tilt=False, psf_binning=3,
                       shift_fixed=None, out_size=(225, 225)):
        if shift_fixed is not None:
            _psf = self.coherent_psf_shifted(
                a, d, phi, normalized=normalized,
                piston_tip_tilt=piston_tip_tilt,
                psf_binning=psf_binning, shift_fixed=shift_fixed,
                out_size=out_size)
            _psf = torch.abs(_psf) ** 2
            return shift_fixed, _psf
        else:
            shift_int, _psf = self.coherent_psf(
                a, d, phi, normalized=normalized,
                piston_tip_tilt=piston_tip_tilt,
                psf_binning=psf_binning, out_size=out_size)
            _psf = torch.abs(_psf) ** 2
...
if __name__ == "__main__":
    ...
    zernike_coeffs = torch.tensor([ 0.0000e+00,  0.0000e+00,  0.0000e+00, -5.6092e-01,  0.0000e+00,
        -5.6877e-01,  1.2308e-01, -8.0722e-01, -1.9861e+00,  1.2015e-02,
         4.3067e-03,  7.6176e-04,  7.7116e-01,  3.3611e-03,  1.1611e-01,
        -4.2975e-02,  5.0776e-03, -1.3879e-01,  8.2825e-02, -3.4058e-02,
        -6.5330e-02], device=device) # thu rush3d003 20251209 roi

    zernike_coeffs = torch.tensor([ 0.        ,  0.        ,  0.        ,  0.7214399 ,  0.        ,
       -0.57937115, -0.01631011,  0.08757537, -0.97343236, -0.03586298,
        0.00239405, -0.01622288,  0.32421264,  0.03242085,  0.04698987,
       -0.00580334,  0.00529809,  0.01333856, -0.02595017, -0.00577183,
       -0.00589736], device=device) # thu rush3d003 20251230 zernike avg
